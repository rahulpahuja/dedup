...
class VideoScannerRepositoryTest {
    ...
    fun `scanVideos returns only valid videos from MediaStore`() = runTest {
        insertVideoToMediaStore("movie.mp4", 50000L, 10000L, "/storage/emulated/0/movie.mp4", "video/mp4")
        insertVideoToMediaStore("clip.mkv", 60000L, 5000L, "/storage/emulated/0/clip.mkv", "video/x-matroska")
        insertVideoToMediaStore("song.mp3", 4000L, 3000L, "/storage/emulated/0/song.mp3", "audio/mpeg")

        val videos = repository.scanVideos(deepScan = false).toList()
    ...
    fun `scanVideos calculates frame hashes when deepScan is true`() = runTest {
        ...
        val videos = repository.scanVideos(deepScan = true).toList()
